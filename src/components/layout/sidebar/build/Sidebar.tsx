...
const Sidebar: React.FC<SidebarProps> = ({ definedFunctions }) => {
  const { lang, t } = useLanguage();
  // Load components list based on selected language
  const categorizedComponents = getComponentsData(lang) as CategoryGroup[];

  const handleDragStart = (
    e: React.DragEvent<HTMLDivElement>,
    component: DraggableSidebarItem
  ) => {
    e.dataTransfer.setData("application/json", JSON.stringify(component));
  };

  // Prepare function call blocks from definedFunctions
  const functionCallBlocks: DraggableSidebarItem[] = definedFunctions.map(
    (func) => ({
          ...
          return (
            <div key={categoryGroup.category} className="mb-4">
              ...
              {categoryGroup.items.map((component) => {
                const compLabel = categorizedComponents.find(
                  (cat) => cat.category === categoryGroup.category
                )?.items.find((item) => item.id === component.id)?.name || component.name;
                return (
                  <div
                    key={component.id}
                    draggable
